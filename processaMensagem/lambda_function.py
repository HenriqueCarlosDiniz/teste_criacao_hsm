...
def lambda_handler(event, context):
    ...
    ############################################################################
    ################ RESGATE O TEMPO DA ULTIMA MENSAGEM ########################
    ############################################################################    
    ...
    if contato.nome == 'receptivo' or contato.nome == 'leads_whatsapp':
        ...
        if resposta_cadastro['error'] != False:
            ...
            print(f"INFO(receptivo:resposta_cadastro)-{id_contato}:", resposta_cadastro)
    ...
    if error == False:
        ...
        return {'statusCode': 400}
    print(f"INFO(resposta_gpt:get_resposta_gpt)-{id_contato}:", resposta_gpt)

    if conversa.get_flag() != int(os.environ['ON_HOLD']):
        ############################################################################
        ##################### SALVA RESPOSTA E FLAG NO DB ##########################
        ############################################################################
        current_datetime = datetime.now(pytz.timezone('America/Sao_Paulo'))
        ...
        #############################################################################
        ########################## LOGICA DE ENCERRAMENTO ###########################
        #############################################################################
        id_conversa = [conversa.id_conversa[0], conversa.id_conversa[1], conversa.id_conversa[2]] #HOMOLOG: usa str no id_conversa1
        ret_enc, resp_enc = logica_Encerramento(id_conversa)
        print(f"ret_enc: {ret_enc}")
        print(f"resp_enc: {resp_enc}") 
        
        if ret_enc:
            #############################################################################
            ###################### ENVIA RESPOSTA AO WPP ################################
            #############################################################################
            if resp_enc['encerramento_ret']:
            ...
            if ret_env is False:
                ...
                formatted_datetime_flag = current_datetime.strftime("%Y-%m-%d %H:%M:%S") 
                deletar_mensagem_anterior(conversa, formatted_datetime_msg)
                #conversa.set_flag("RESPONDER", formatted_datetime_flag)
            else:
                current_datetime = datetime.now(pytz.timezone('America/Sao_Paulo'))
...
def deletar_mensagem_anterior(conversa, datetime_msg):
    delete_query = f"""DELETE FROM MENSAGEM 
...
def atualiza_horario_mensagem(conversa, formatted_datetime_msg, formatted_datetime_mensagem_atualizada):
    query = f"""UPDATE `MENSAGEM` SET `MENSAGEM_DATETIME` = '{formatted_datetime_mensagem_atualizada}'
    WHERE (`TELEFONE` = {conversa.contato.id_contato}) 
    and (`ID_CAMPANHA` = '{conversa.campanha}') 
    and (`DATETIME_CONVERSA` = '{conversa.datetime_conversa}') 
