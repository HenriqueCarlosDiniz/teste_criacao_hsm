from sqlalchemy import create_engine, Table, MetaData, pool, text
import pandas as pd
import os
...
class DatabaseConnection:
    ...
    def __new__(cls):
        if cls._instance is None:
            ...
            connection_string = f"mysql+pymysql://{os.environ.get('DB_USERNAME')}:{os.environ.get('DB_PASSWORD')}@{os.environ.get('DB_HOST')}/{os.environ.get('DB_DATABASE')}"
...
#             conn.commit()
#             final_connections = DatabaseConnection.get_active_connections()
#             print(f"Active connections before query: {initial_connections}")
#             print(f"Active connections after query: {final_connections}")
#     except Exception as e:
#         DatabaseConnection.dispose_engine()
